...
#%% Making a list of MODIS .tif files (from A_Reprojecting and cropping MODIS.py) and a list of tile. tif files(from B_Splitting DEM.py)
list1 = natsorted(glob.glob(r'D:\Archive\2017-2018\Thesis\Data\MODIS MOD10A1\Cropped data\*.tif'))
dempath = natsorted(glob.glob(r'D:\Archive\2017-2018\Thesis\Data\GMTED2010\Tiles\*.tif'))
...
for i in range(len(list1)):
    tic = time.time()
    for j in range(len(list2)): #Use the total amount of DEM tiles
        modpath = list1[i] 
        tilepath = list2[j]
        ...
        n2 = p2.stem
        name = str(n1+'_'+n2)
        tilenolist = (re.findall(r'tile_(\d+).tif', tilepath))
        tileno = int(tilenolist[0])
        
        # Crop mod to tile extent
        dstpath = "D:\\Archive\\2017-2018\\Thesis\\Data\\MODIS MOD10A1\\Tiled data\\" + name + ".tif"
        mod_tile = SetCRSRes(modpath, dstpath, tile_crs, tile_transform, tile_width, tile_height)
        
        # Read MOD raster as array
        with rasterio.open(dstpath) as mod:
            ...
            #Proceed onlu if cloud pixel amount does not exceed threshold
            if cper >= 0.7:
                RSLE = cloud
                RSL = np.nan
                b.append([date, tileno, RSLE, RSL])
                continue
...
tablename = 'D:\\Archive\\2017-2018\\Thesis\\Data\\MODIS MOD10A1\\Tiled data\\tiletab.txt'
np.savetxt(tablename, b, fmt=['%.d','%2.f','%4.F','%6.F'], delimiter=',', newline='\r\n')
